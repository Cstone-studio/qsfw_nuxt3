    ...
    >
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button type="primary" @click.prevent="editUser(formRef)">
            Confirm
          </el-button>
        </div>
      </template>
...
const openDialog = async (row: User) => {
  await userStore.getUserDetail(row.id);
  copyProperties(userStore.userDetail, data.form);
...
const editUser = async (formRef: FormInstance | undefined) => {
  if (!formRef) return;
